...
if($age >= 15) { // >= are the comparison symbols to check if the user is 15 or above. <= would do the complete opposite, thus: check if the user is below 15 or above.
    ...
    echo "You need to be 15+ to download the game."; // echoes an error message.
}
```